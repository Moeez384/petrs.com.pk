...
<div class="card ml-3 mr-4 bg-white border border-0 rounded-0 shadow" style="background-color: #05386B;">
	<div class="card-body table-responsive">
		<h3 style="color: #05386B;" class="text-center">NEWS LIST</h3>
	</div>
</div>
<br>
...
<e style="font-size: 14px; color: #000">
	<div class="card-body table-responsive">
		<button style="float:right; background-color: #05386B; color: white"  class="btn btn-sm rounded-0 mb-4" name="add">Add News</button>
		<table id="myTable" class="table table-bordered table-hover" style="width: 100%; color:black">
			...
			<tr style="background-color: whitesmoke">
				...
				<th class="text-center"><i class="fas fa-building"></i>&nbsp;Last Updated</th>
			</tr>
		</thead>
		<tr>
		<?php
		$all_news = $this->ProjectsModel->fetch_all_news();
		if($all_news) {
			foreach($all_news as $an) {
		?>	
			<td class="text-center"><a href="<?php echo base_url();?>admin/News/edit_news?id=<?php echo $an->id?>" title="update"><i class="fas fa-edit text-primary" style="font-size: 18px"></i></a>
			</td>
			<td class="text-center"><?php echo $an->title;?></td>
			<td class="text-center"><?php echo (strlen($an->details) > 13) ? substr($an->details,0,18).'...' : $an->details;?></td>
			<td class="text-center"><?php echo $an->created_date;?></td>
		</tr>
		<?php }} ?>
		</table>
	</div>
