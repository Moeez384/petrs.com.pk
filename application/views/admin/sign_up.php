...
body {
      background-image: url("<?php echo base_url();?>assets/images/Admin-Background.jpg");
      object-fit: contain;
    }
/*
...
<body>
	<div class="container">
		<div class="card center" style="width: 38%; height: 90%; background-color: ghostwhite">
			<div class="card-body">
				<form method="post" action="<?php echo base_url()?>admin/Signin/save_signup">

				<h4 class="text-center">SIGNUP AS ADMIN </h4>
						<div class="form-group has-feedback mt-4">
           <input type="text" name="name" class="form-control" placeholder="name" style="font-size: 15px" />
          <span class="glyphicon glyphicon-envelope form-control-feedback"></span> 
        </div>
        		<div class="form-group has-feedback mt-4">
           <input type="text" name="lastname" class="form-control" placeholder="last name" style="font-size: 15px" />
          <span class="glyphicon glyphicon-envelope form-control-feedback"></span> 
        </div>
					<div class="form-group has-feedback mt-4">
      ...
      <div class="form-wrapper has-feedback text-center">
		...
		<br>
			<a href="<?php echo base_url()?>admin/Signin" style="font-size: 14px; font-style: italic; text-decoration: underline; padding-left: 31%">Already registered? Sign In</a>
	<br>
		<hr>
         <div class="lockscreen-footer text-center text-dark" style="font-size: 13px;">
            <?php echo date("Y"); ?> © <?php echo 'PETRS'; ?>
        </div>
</form>
			</div>
		</div>
	...
	if($this->input->post('btn')) {
		...
		if($pass) {
			if(password_verify($this->input->post('password'), $pass)) {
				$get_id = $this->ProjectsModel->get_admin($email);
				$_SESSION['id'] = $get_id; 
