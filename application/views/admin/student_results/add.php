  ...
  <div class="card border border-0 rounded-0 shadow ml-5 mr-5">
    <div class="container">
      ...
      <div class="card-body">
        ...
        <?php } ?>
        <form method="post" action="<?php echo base_url()?>admin/Results/save" enctype="multipart/form-data">
        <div class="container ml-2">
        <div class="row"> 
          <div class="col col-sm">
            <p class="mt-3 text-dark" style="font-size: 17px">Result Name:
            <select class="form-control mt-1 rounded-3" id="txt-name" type="text" name="name" style="width: 100%">
              <?php 
              $completed_projects = $this->ProjectsModel->fetch_completed_projects();
              if($completed_projects) { ?>
                <option>Select Result</option>
              <?php foreach($completed_projects as $pr) {
              ?>
          ...
          <!--   <input class="form-control mt-1 rounded-3" id="txt-name" type="text" name="name" style="width: 100%"> -->
          </p>
        ...
        <div class="row">
          <div class="col col-sm">
            <button style="float:right" class="btn btn-primary rounded-3 pl-3 pr-3" name="save" onclick="return add_result()"><i class="fa fa-check-square" aria-hidden="true"></i> Save</button>
          </div>
        </div>
      </div>
    </div>
